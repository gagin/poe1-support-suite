---
name: poe2-analyze
description: Reuse a recent Path of Exile 2 character snapshot or, when stale or explicitly requested, pull the character from the account; then expand as needed, analyze the build, and record findings to the notes vault. Use when asked to import/analyze/review a PoE2 character (e.g. "analyze my PoE2 character X", "pull toteBotato", "review my Warrior").
---

# PoE2: reuse or refresh → analyze → record
...
Before calling the account API, find the newest usable expanded snapshot for the
character in `build_snapshots_poe2/`.

- Reuse it when it is **no more than one hour old**.
- Pull only when the user explicitly asks to pull/refresh, the newest snapshot is
  **more than one hour old**, or no usable snapshot exists.
- Do not repull merely because the user asks a follow-up analysis question in the
  same session.
- If the API is known to lag a live item or measurement, treat the user's live
  observation as authoritative. A fresh pull inside the one-hour window is not a
  remedy for known API staleness.

When reusing a snapshot, skip directly to analysis.

## 2. Pull the character when required (OAuth)
...
Or run the conditional pull and expansion together:
`make snapshot2 CHARACTER=<Name>`.
